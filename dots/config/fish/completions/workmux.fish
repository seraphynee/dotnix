...
complete -c workmux -n "__fish_workmux_using_subcommand help; and not __fish_seen_subcommand_from add open close resurrect merge rebase rename remove list path send capture status wait run reap-agents sync-files init setup uninstall docs changelog update sidebar _sidebar-run _sidebar-sync _sidebar-reflow _sidebar-reflow-all _sidebar-daemon dashboard config claude sandbox set-window-status set-base _exec last-done last-agent host-exec clipboard-read completions _complete-branches _complete-handles _complete-git-branches _complete-agent-targets _check-update help" -f -a sidebar -d 'Control a live agent status sidebar in tmux'
